...
// DiR is the default; no separate constructors needed
...
fn bench_compress2(c: &mut Criterion) {
    ...
    // DiR is default, already covered by the two benches above
    group.finish();
}
...
fn bench_merkle_tree_build(c: &mut Criterion) {
    ...
    for &n_leaves in &[1usize << 8, 1 << 10] {
        ...
        // No separate DiR benches; default covers both patterns
    }
    group.finish();
}
