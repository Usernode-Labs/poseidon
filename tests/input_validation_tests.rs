...
fn test_memory_usage_bounds() {
    ...
    assert!(
        max_memory_estimate < 100_000_000,
...
fn test_invalid_utf8_handling() {
    ...
    let invalid_utf8 = vec![0x80, 0x81, 0x82, 0x83, 0xFF, 0xFE, 0xFD, 0xC0, 0x80];
...
fn test_concurrent_input_validation() {
    ...
    for (thread_id, success, size) in results_guard.iter() {
        assert!(
            *success,
            "Thread {} failed to process {} bytes",
            thread_id, size
        );
    }
}
...
fn test_validation_performance_consistency() {
    ...
    assert!(
        ...
        "Performance degradation detected: {:.2}x slower",
        performance_ratio
    );
}
